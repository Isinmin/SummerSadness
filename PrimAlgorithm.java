...
if (scan.hasNextInt()) {
    ...
    public boolean executeStepAlgorithm() {
        ...
        if (!intree[v]) {
            ...
            while (p != null) {
                ...
                if (w != 0) {
                    ...
                    if (distance[w] > weight && !intree[w]) {
                        distance[w] = weight;
                        parent[w] = v;
                        textInfo += "Вес ребра ( "+v+","+w+" ) меньше чем тот, который уже записан ("+weight+") и вершина "+w+" еще не добавлена в дерево. Запоминаем новый вес.\n";
                    }
                else {
                    if (distance[w] > weight)
                        textInfo += "Вес ребра ( "+v+","+w+" ) больше или равен тому ("+distance[w]+"), который уже записан ("+weight+").\n";
            ...
            textInfo += "Все вершины добавлены в МО \nАлгоритм закончил работу\n";
            return true;
        }
    }
